...
export function mountStoryViewer({
  ...
  header.innerHTML = `
    <div class="flex items-center gap-3">
      <img src="${story.user?.avatar ?? '/frendz/images/avatars/placeholder.jpg'}" alt="${story.user?.name ?? 'User'}" class="h-10 w-10 rounded-full object-cover border-2 border-white/30" />
      <div>
    ...
    <div class="flex items-center gap-2">
      <button type="button" class="h-9 w-9 rounded-full bg-white/10 backdrop-blur flex items-center justify-center active:scale-95 transition" data-action="mute">
        <img src="${muted ? '/frendz/icons/muted.svg' : '/frendz/icons/volume.svg'}" alt="Toggle audio" class="h-4 w-4" />
      </button>
      <button type="button" class="h-9 w-9 rounded-full bg-white/10 backdrop-blur flex items-center justify-center active:scale-95 transition" data-action="close">
        <img src="/frendz/icons/close.svg" alt="Close" class="h-4 w-4" />
      </button>
    </div>
  `;
  ...
  muteButton.addEventListener('click', () => {
    muted = !muted;
    muteButton.innerHTML = `<img src="${muted ? '/frendz/icons/muted.svg' : '/frendz/icons/volume.svg'}" alt="Toggle audio" class="h-4 w-4" />`;
    if (currentMedia && currentMedia.tagName === 'VIDEO') {
