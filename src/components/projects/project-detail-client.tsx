  ...
  useEffect(() => {
    ...
    const compute = async () => {
      ...
      const ratios = await Promise.all(
        items.map(
          (it) =>
            new Promise<{ id: string; url: string; ratio: number }>(
              (resolve) => {
                const img = new Image();
                img.onload = () => {
                  const r =
                ...
                img.onerror = () =>
                  resolve({ id: it.id, url: it.url, ratio: 1.7778 });
                img.src = it.url;
              }
            )
        )
      ...
      const pushRow = () => {
        ...
        const baseWidth = row.reduce(
          (sum, it) => sum + it.ratio * targetRowHeight,
          0
        );
      ...
      for (const it of ratios) {
        ...
        if (
          rowWidth + w + (row.length > 0 ? gap : 0) > containerWidth &&
          row.length > 0
        ) {
          pushRow();
        }
...
export default function ProjectDetailClient({
  ...
  const allCategories =
    project.project_categories?.map((pc) => pc.categories) ||
  ...
  const embedInfo = useMemo(
    () => getEmbedInfo(project.cover_video_url || ""),
    ...
    return blocks
      .filter(
        (b) =>
          ...
          (b.block_type === "gallery" && b.content === "__before_after__")
      )
      .map((b) => ({
        ...b,
  ...
  const gridItems = useMemo(() => {
    const items = [
      ...
      ...blocks.flatMap((b) =>
        b.block_type === "image" && b.media_url
          ? [{ id: `b-${b.id}`, url: b.media_url }]
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div
        className="w-full lg:h-screen lg:overflow-hidden"
      ...
      >
        <div className="flex flex-col lg:flex-row lg:h-full gap-0">
          ...
          <section className="shrink-0 w-full lg:w-[40%]">
            <div
              ref={leftRef}
              className="h-[calc(100vh-5rem)] lg:h-full lg:min-h-0 overflow-y-auto flex flex-col py-8 pr-6"
            >
              <Link
              ...
              <dl className="grid grid-cols-2 gap-x-8 gap-y-6 text-sm mb-6">
                {project.client && (
                  <div>
                    <dt className="font-semibold uppercase tracking-widest text-xs text-muted-foreground/80 mb-2">
                      Client
                    </dt>
              ...
              {(project.content || textBlocks.length > 0) && (
                <div className="space-y-8 mt-8 pr-2">
                  ...
                  {textBlocks.length > 0 && (
                    <ContentBlocksRenderer blocks={textBlocks as any} />
                  )}
                </div>
              )}
          ...
          <aside
            ref={rightRef}
            className="w-full lg:w-[60%] lg:min-h-0 lg:h-full lg:overflow-y-auto scrollbar-hide relative"
          >
                ...
                <div className="relative">
                  ...
                  {project.cover_video_url && (
                    <button
                      ...
                      aria-label={
                        isPlaying ? "Pause cover video" : "Play cover video"
                      }
                    >
                      {isPlaying ? (
