...
export function VisualDesignLayout({ project, media, tags, blocks }: VisualDesignLayoutProps) {
  ...
  const categorySlug = project.project_categories?.[0]?.categories?.slug || 'visual-design';
  const backgroundColor = project.background_color || '#0b0b0c';
  const textColor = (project as any).text_color || '#ffffff';

  const imageMedia = useMemo(() => {
    const filtered = media.filter((item) => isRenderableImage(item.url, item.media_type));
  ...
  const sections = useMemo(() => {
    const combined: Array<{ type: 'image' | 'text'; data: any; order: number }> = [];
  ...
  const images = useMemo(() => sections.filter(s => s.type === 'image').map(s => s.data as MediaItem), [sections]);
  ...
  useEffect(() => {
    if (!vh || images.length === 0) { setSlideWidths([]); return; }
    let cancelled = false;
    Promise.all(
      images.map((m) => new Promise<number>((resolve) => {
    ...
    ).then((widths) => { if (!cancelled) setSlideWidths(widths); });
    return () => { cancelled = true; };
  }, [images, vh]);

  const overlaysBySlide = useMemo(() => {
    const imagePositions: number[] = [];
    sections.forEach((s, i) => { if (s.type === 'image') imagePositions.push(i); });
  ...
  const computedTotal = slideWidths.length === images.length && images.length > 0
  ...
  const offsets = useMemo(() => {
    if (slideWidths.length === images.length && images.length > 0) {
      const arr: number[] = [];
      let acc = 0;
      for (let i = 0; i < slideWidths.length; i++) { arr.push(acc); acc += slideWidths[i]; }
      return arr;
    }
  ...
  const contentBlocksHtml = useMemo(() => {
    ...
    Object.keys(overlaysBySlide).forEach((k) => {
      ...
      if (blocksArr && blocksArr.length) {
        blocksArr.filter((b: any) => b.block_type !== 'video' && b.block_type !== 'before_after' && b.block_type !== 'testimonial').forEach((b: any) => {
          if (b.block_type === 'quote') {
            blocks.push(`<blockquote class="border-l-4 border-current pl-6 py-4 italic text-xl opacity-90">${String(b.content || '')}</blockquote>`);
          } else {
    ...
    return blocks.join('<div class="my-8 border-t border-current opacity-30"></div>');
  }, [overlaysBySlide]);
  ...
  const testimonialsHtml = useMemo(() => {
    ...
    Object.keys(overlaysBySlide).forEach((k) => {
      ...
      const blocksArr = overlaysBySlide[idx];
      if (blocksArr && blocksArr.length) {
        blocksArr.filter((b: any) => b.block_type === 'testimonial').forEach((b: any) => {
          try {
            const testimonial = b.content ? JSON.parse(b.content) : {};
            testimonials.push(`
              <div class="bg-current/5 rounded-lg p-6 my-4">
                <p class="italic text-base mb-4 leading-relaxed">"${testimonial.text || ''}"</p>
                <div class="flex items-center gap-3">
                  ${testimonial.image ? `<img src="${testimonial.image}" alt="${testimonial.name}" class="w-12 h-12 rounded-full object-cover" />` : ''}
                  <div>
                    <p class="font-bold text-sm">${testimonial.name || ''}</p>
                    ${testimonial.role ? `<p class="text-xs opacity-70">${testimonial.role}</p>` : ''}
                  </div>
                </div>
              </div>
            `);
  ...
  const testimonialsY = useTransform(smoothProgress, [0, 0.25, 0.35], [20, 10, 0]);
  ...
  const rowItems = useMemo(() => {
    const items: Array<{ type: 'image' | 'video' | 'before_after'; img?: MediaItem; block?: any; hasCoverVideo?: boolean }> = [];
    
    // Add cover image (with cover video flag)
    if (images.length > 0) {
      items.push({ type: 'image' as const, img: images[0], hasCoverVideo: !!coverVideoUrl });
    }
    ...
    for (let i = 1; i < images.length; i++) {
      ...
      if (blocksArr && blocksArr.length) {
        const videoBlocks = blocksArr.filter((b: any) => b.block_type === 'video');
        videoBlocks.forEach((vb: any) => {
          items.push({ type: 'video' as const, block: vb });
        });
        
        const beforeAfterBlocks = blocksArr.filter((b: any) => b.block_type === 'before_after');
  ...
  const rowOffsets = useMemo(() => {
    const offs: number[] = [];
    let acc = 0;
    for (const w of rowWidths) { offs.push(acc); acc += w; }
    return offs;
  }, [rowWidths]);

  const rowTotal = useMemo(() => rowWidths.reduce((a, b) => a + b, 0), [rowWidths]);
  ...
  if (vw > 0 && vw < 768) {
    return (
      <>
        ...
        <div className="relative z-[9999] min-h-screen w-full" style={{ backgroundColor, color: textColor }}>
          {/* Logo + Close */}
          <Link
            href="/"
          ...
          >
            <Image src="/Anomaly.png" alt="Crafted Anomaly" width={40} height={40} className="h-10 w-10 object-contain drop-shadow-lg" priority />
          </Link>
        <Link
          href={`/${categorySlug}`}
        ...
        <div className="px-6 py-20 pb-12" style={{ color: textColor }}>
          {project.project_type && (
            <div className="text-[10px] uppercase tracking-[0.3em] opacity-80 mb-2">{project.project_type}</div>
          )}
          <h1 className="text-3xl md:text-4xl font-black leading-tight mb-4">{project.title}</h1>
          {project.blurb && <p className="text-sm leading-relaxed opacity-80 mb-6">{project.blurb}</p>}
          
          <div className="text-xs opacity-70 space-y-1">
            {categoryInfo && (
              <div><b>Category:</b> {categoryInfo.name}</div>
            )}
            {project.client && <div><b>Client:</b> {project.client}</div>}
            {project.year && <div><b>Year:</b> {project.year}</div>}
            {project.role_en && <div><b>Role:</b> {project.role_en}</div>}
          </div>
        </div>
        ...
        <div className="space-y-0">
          {rowItems.map((item, index) => {
            if (item.type === 'image' && item.img) {
              ...
              const videoInfo = item.block.content || item.block.media_url || '';
              return (
                <div key={`video-${index}`} className="relative w-full bg-black">
                  <div className="relative w-full" style={{ aspectRatio: '16/9' }}>
                    {videoInfo.includes('youtube.com') || videoInfo.includes('youtu.be') ? (
                      <iframe
                        src={videoInfo.replace('watch?v=', 'embed/').replace('youtu.be/', 'youtube.com/embed/')}  
                      ...
                      <iframe
                        src={videoInfo.replace('vimeo.com/', 'player.vimeo.com/video/')}
                        className="w-full h-full"
                        allow="autoplay; fullscreen; picture-in-picture"
                        allowFullScreen
                      />
                    ) : (
                      <video src={videoInfo} controls className="w-full h-full" />
                    )}
                  </div>
                </div>
              );
        ...
        {contentBlocksHtml && (
          <div className="px-6 py-10" style={{ color: textColor }}>
            <div className="prose prose-sm max-w-none" style={{ color: textColor }} dangerouslySetInnerHTML={{ __html: contentBlocksHtml }} />
          </div>
        )}

        {/* Testimonials */}
        {(project as any).testimonials && Array.isArray((project as any).testimonials) && (project as any).testimonials.length > 0 && (
          <div className="px-6 py-10">
            <div className="flex flex-wrap gap-4 justify-center">
              {((project as any).testimonials as string[]).map((imgUrl: string, idx: number) => (
                <img 
                  key={idx}
                  src={imgUrl} 
                  alt={`Award ${idx + 1}`} 
  ...
  return (
    <>
      ...
      {/* Fixed viewport */}
      <div className="fixed inset-0 z-[10000] overflow-hidden pointer-events-none" style={{ backgroundColor }}>
        {/* Logo + Close */}
        <Link
          href="/"
          ...
          <div className="relative h-full pointer-events-auto" style={{ color: textColor }}>
            ...
            {testimonialsHtml && testimonialsHtml.length > 0 && (
              <>
                ...
                <div className="space-y-4" style={{ color: textColor }} dangerouslySetInnerHTML={{ __html: testimonialsHtml }} />
              </>
            )}
            {(project as any).testimonials && Array.isArray((project as any).testimonials) && (project as any).testimonials.length > 0 && (
              <motion.div
        ...
        <motion.div
          ...
          style={{ x: xRow, width: rowTotal, willChange: 'transform', left: infoPanelWidth }}
        >
          {rowItems.map((it, idx) => {
...
function ImageSlide({
  ...
  const leftClip = useTransform(viewportX, (v: number) => Math.max(0, Math.min(width, v - offset)));
  ...
  const getVideoInfo = (content: string) => {
    ...
    if (content.includes('youtube.com') || content.includes('youtu.be')) {
      ...
      if (match && match[1]) {
        return { type: 'youtube', embedUrl: `https://www.youtube.com/embed/${match[1]}?autoplay=1&controls=1&rel=0` };
      }
    }
    ...
    if (content.includes('vimeo.com')) {
      ...
      if (match && match[1]) {
        return { type: 'vimeo', embedUrl: `https://player.vimeo.com/video/${match[1]}?autoplay=1` };
      }
    }
  ...
  return (
    <div className="absolute top-0 h-screen" style={{ left: offset, width, zIndex: 10 }}>
      <motion.div className="absolute inset-0" style={{ clipPath, willChange: 'clip-path' }}>
        <div className="absolute inset-0 bg-left bg-no-repeat" style={{ backgroundImage: `url(${img.url})`, backgroundSize: 'auto 100%' }} />
        ...
        {coverVideoUrl && (
          <>
            {showVideo && videoInfo ? (
              ...
              >
                <svg className="w-8 h-8 text-black ml-0.5" fill="currentColor" viewBox="0 0 20 20">
...
function BeforeAfterSlide({
  ...
  return (
    <div className="absolute top-0 h-screen" style={{ left: offset, width, zIndex: 15 }}>
      <motion.div 
        ref={containerRef}
      ...
      >
        ...
        <div 
          ...
          style={{ left: `${sliderPosition}%`, transform: 'translateX(-50%)', transition: isDragging ? 'none' : 'left 0.1s ease-out' }}
        >
...
function VideoSlide({
  ...
  return (
    <div className="absolute top-0 h-screen" style={{ left: offset, width, zIndex: 15 }}>
      <motion.div className="absolute inset-0 pointer-events-auto" style={{ clipPath, willChange: 'clip-path' }}>
...
function ContentBlockSlide({
  ...
  const y = useTransform(scrollProgress, [start - 0.1, start, end, end + 0.1], [100, 0, 0, -100]);
  const opacity = useTransform(scrollProgress, [start - 0.1, start, end, end + 0.1], [0, 1, 1, 0]);
